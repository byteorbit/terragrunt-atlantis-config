...
func RunWithFlags(generateCmd *cobra.Command, filename string, args []string) ([]byte, error) {
...
func runTest(t *testing.T, goldenFile string, args []string) {
	...
	contentBytes, err := RunWithFlags(generateCmd, filename, allArgs)
	content := &generate.AtlantisConfig{}
	...
	goldenContents := &generate.AtlantisConfig{}
...
`)
	...
	content, err := RunWithFlags(generateCmd, filename, []string{
		"--output",
		filename,
		"--root",
		filepath.Join("test_examples", "basic_module"),
